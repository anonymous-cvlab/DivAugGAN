...
class IPR():
    ...
    def precision_recall_density_coverage(self, subject):
        '''
        Compute precision, recall, density and coverage for given subject
        reference should be precomputed by IPR.compute_manifold_ref()
        args:
            subject: path or images
                path: a directory containing images or precalculated .npz file
        ...
        manifold_subject = self.compute_manifold(subject)
        prdc_dict = compute_prdc(manifold_subject, self.manifold_ref.features, nearest_k) 
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        precision, recall, density, coverage = ipr.precision_recall_density_coverage(args.path_fake)
